...
class Recommender:
    ...
    def getDateWeights(self,userId):
        ...
        for index,order in orderHistory.iterrows():
            ...
            orderTimestamp = time.mktime(datetime.datetime.strptime(orderDate,'%d/%m/%Y').timetuple())
        ...
        for index,order in orderHistory.iterrows():
            ...
            orderDate = order['date']
            orderTimestamp = time.mktime(datetime.datetime.strptime(orderDate,'%d/%m/%Y').timetuple())
            dateData[currentPid] = 0.2 if float((orderTimestamp-minDate)/(maxDate-minDate))== 0 else float((orderTimestamp-minDate)/(maxDate-minDate))
        return dateData
    ...
    def getSingleProductRecommendation(self,productId):
        ...
        for i in range(0,10):
            print(productCorr[newArray[i][1]],self.productList[newArray[i][1]])
    ...
    def getProductRecommendation(self,userId):
        ...
        searchQueries = self.browserHistory.loc[self.browserHistory['uid'] == userId]['search'].values.tolist()
        print(searchQueries)
